...
const SLUG = "ejder-vs-ai";

export const metadata = {
  title: "EJDER VS. AI — Copa del Mundo 2026",
  description: "Bröderna Ejder mot Lindmarker Model och El Bizteka. Människa mot maskin.",
};

export default async function EjderVsAiPage() {
  const rivalry = getRivalry(SLUG)!;
  const { userId } = await auth();
  const [{ rowsByName, anyScored }, summary, myVote] = await Promise.all([
    getRivalryData(),
    getVoteSummary(SLUG),
    userId ? getMyVote(SLUG, userId) : Promise.resolve(null),
  ]);
  const sides = rivalry.sides.map((s) => resolveSide(s, rowsByName));
